...
// between the end-nodes that would be present in a complete binary tree extending down to that level are also counted 
...
// 2. The idea is to give index to each node (considering index of intermediate NULL nodes too) starting from root as 0 so that
//  we can easily find no. of nodes at each level by (rightIndex-leftIndex + 1)
